...
class TemporalUnet(nn.Module):
    ...
    def forward(
        ...
        if self.is_conditioned_label:
            ...
            label_embed = self.label_mlp(label)
            if use_dropout:
                mask = self.mask_dist.sample(sample_shape=(label_embed.size(0), 1)).to(
                    label_embed.device
                )
                label_embed = mask * label_embed
            if force_dropout:
                label_embed = 0 * label_embed
            t = torch.cat((t, label_embed), dim=-1)
