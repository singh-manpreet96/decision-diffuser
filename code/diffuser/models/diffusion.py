...
class GaussianDiffusion(nn.Module):
    ...
    def p_mean_variance(self, x, cond, t, returns=None, labels=None):
        ...
        if self.returns_condition:
            ...
            epsilon_cond = self.model(x, cond, t, returns, labels, use_dropout=False)
            epsilon_uncond = self.model(x, cond, t, returns, labels, force_dropout=True)
    ...
    @torch.no_grad()
    def p_sample(self, x, cond, t, returns=None, labels=None):
        b, *_, device = *x.shape, x.device
        model_mean, _, model_log_variance = self.p_mean_variance(
            x=x, cond=cond, t=t, returns=returns, labels=labels
        )
    ...
    def p_sample_loop(
        ...
        for i in reversed(range(0, self.n_timesteps)):
            ...
            x = self.p_sample(x, cond, timesteps, returns, lables=labels)
            x = apply_conditioning(x, cond, self.action_dim)
    ...
    def conditional_sample(
        self, cond, returns=None, horizon=None, labels=None, *args, **kwargs
    ):
        """
        ...
        return self.p_sample_loop(shape, cond, returns, labels, *args, **kwargs)
    ...
    def p_losses(self, x_start, cond, t, returns=None, labels=None):
        noise = torch.randn_like(x_start)
        ...
        x_recon = self.model(x_noisy, cond, t, returns, labels)
...
class GaussianInvDynDiffusion(nn.Module):
    def __init__(
        ...
        loss_discount=1.0,
        loss_weights=None,
        is_conditioned_scalar=False,
        is_conditioned_label=False,
        condition_guidance_w=0.1,
        ar_inv=False,
        ...
        self.is_conditioned_scalar = is_conditioned_scalar
        self.is_conditioned_label = is_conditioned_label
        self.condition_guidance_w = condition_guidance_w
    ...
    def p_mean_variance(self, x, cond, t, returns=None, labels=None):
        if self.is_conditioned_scalar or self.is_conditioned_label:
            # epsilon could be epsilon or x0 itself
            epsilon_cond = self.model(x, cond, t, returns, labels, use_dropout=False)
            epsilon_uncond = self.model(x, cond, t, returns, labels, force_dropout=True)
            epsilon = epsilon_uncond + self.condition_guidance_w * (
    ...
    def p_sample_loop(
        ...
        for i in reversed(range(0, self.n_timesteps)):
            timesteps = torch.full((batch_size,), i, device=device, dtype=torch.long)
            x = self.p_sample(x, cond, timesteps, returns, labels)
            x = apply_conditioning(x, cond, 0)
    ...
    def loss(self, x, cond, returns=None, labels=None):
        if self.train_only_inv:
            ...
            diffuse_loss, info = self.p_losses(
                x[:, :, self.action_dim :], cond, t, returns, labels
            )
            # Calculating inv loss
            x_t = x[:, :-1, self.action_dim :]
            a_t = x[:, :-1, : self.action_dim]
    ...
    def forward(self, cond, returns=None, labels=None, *args, **kwargs):
        return self.conditional_sample(
            cond=cond, returns=None, labels=None, *args, **kwargs
        )
